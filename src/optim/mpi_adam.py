...
class MpiAdamOptimizer(tf.train.AdamOptimizer):
    ...
    def compute_gradients(self, loss, var_list, **kwargs):
        ...
        flat_grad = tf.concat(
            [tf.reshape(g, (-1,)) for g, v in grads_and_vars], axis=0) * self.mpi_rank_weight
        ...
        # Counts how many times _collect_grads has been called
        countholder = [0]
        ...
        avg_flat_grad = tf.py_func(
            _collect_grads, [flat_grad, stat], tf.float32)
...
def test_nonfreeze():
    ...
    update_op = MpiAdamOptimizer(
        comm=MPI.COMM_WORLD, learning_rate=stepsize).minimize(loss)
